...
def create_user(name, image_file):
    ...
    print("id : " + str(id))
    if not encode_images(image_file, (name + "." + str(id))):
        status = {"status": False, "message": "Unable To Detect Face In Image."}
        return status
    status = add_user(image_file, name, id)
    return status
...
def encode_images(image, name):
    ...
    print("inside encode :" + str(image))
    if image is not None:
        image = face_recognition.load_image_file(image)
        print("inside image check :" + str(image))
